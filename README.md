...
![OCSF fundamentals](./images/ocsf_fundamentals.png)
...
3. **Custom source mappings**: Mapping files are stored in the `mappings` folder under `cdk/transformation-function`.
...
The solution uses mapping files (`cdk/transformation-function/mappings/<custom-source>.json`) to transform logs to OCSF format. Here's an example of the mapping structure:
